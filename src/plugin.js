...
(function () {
  ...
  function initReveal(userCfg) {
    ...
    Reveal.initialize(cfg).then(function () {
      ...
      // Reveal.js sections have height:auto — we must set an explicit
      // max-height so overflow-y:auto has a boundary to scroll within.
      var slideHeight = Reveal.getConfig().height || 720;
      var slideMargin = Reveal.getConfig().margin || 0.08;
      var maxH = Math.floor(slideHeight * (1 - slideMargin * 2)) + 'px';
